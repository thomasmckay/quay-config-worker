...
def process_resources(resources):
    ...
    for resource in resources:
        ...
        if repository is None:
            response.append(
                "Repository '%s/%s' does not exist" % (p_project, p_repository)
            )
        ...
        if p_state == "present":
            args = [
                ...
                "copy",
                "--dest-creds",
                "%s:%s" % (p_user, authentication.encrypt_user_password(p_password)),
                "--src-tls-verify=%s" % p_src_tls_verify,
                "--dest-tls-verify=false",
                "docker://%s:%s" % (p_src_image, p_src_tag),
                "docker://%s/%s/%s:%s"
                % (app.config["SERVER_HOSTNAME"], p_project, p_repository, p_tag),
            ]
            ...
            if job.returncode != 0:
                ...
                return {"failed": True, "changed": changed, "msg": response}, 400
            else:
                changed = True
                response.append(
                    "Source '%s:%s' synced.\nSTDOUT: %s\nSTDERR: %s"
...
def jwt_bearer_token(username, project, repository):
    ...
    context = {}
    context.update({CLAIM_TUF_ROOTS: None, CLAIM_TUF_ROOT: DISABLED_TUF_ROOT})
    subject = username
    access = [
        {
            "type": "repository",
    ...
    auth_token = generate_bearer_token(
        audience, subject, context, access, TOKEN_VALIDITY_LIFETIME_S, instance_keys
    )
