...
def process(resources):
  ...
  for resource in resources:
    ...
    if repository is None:
      return {
        "failed": True,
        "msg": "Repository '%s/%s' does not exist" % (p_project, p_repository)
      }, 400
    ...
    if image is None:
      print json.dumps({
          "failed": False,
          "msg": "Image '%s' does not exist" % p_image_id
      })
      sys.exit(1)
    ...
    if p_state == 'absent':
      ...
      if tag is None:
        tag = model.tag.create_or_update_tag(p_project, p_repository, p_name, image.docker_image_id)
      ...
      signature_kind = "gpg2"
      storage = model.image.find_derived_storage_for_image(tag, storage_transformation, storage_metadata)
