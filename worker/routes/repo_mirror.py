...
from data.database import (Repository, User, RepoMirrorRule, RepoMirrorConfig, RepoMirrorRuleType,
                           RepoMirrorStatus, RepositoryState)
...
def process(resources):
  ...
  for resource in resources:
    ...
    p_external_registry = resource['external_registry']
    p_external_namespace = resource['external_namespace']
    p_external_repository = resource['external_repository']
    ...
    repository = model.repository.get_repository(p_internal_namespace, p_internal_repository)
    if repository is None:
      return {
        "failed": True,
        "msg": "Destination repository '%s/%s' does not exist" % (p_internal_namespace, p_internal_repository)
      }, 400
    ...
    if p_state == 'present':
      ...
      if rule is None or config is None:
        ...
        rule = RepoMirrorRule.create(repository=repository,
                                     rule_type=RepoMirrorRuleType.GLOB_CSV,
                                     rule_value=p_external_tag
        )
        config = RepoMirrorConfig.create(repository=repository,
                                         root_rule=rule,
                                         internal_robot=robot,
                                         external_registry=p_external_registry,
                                         external_namespace=p_external_namespace,
                                         external_repository=p_external_repository,
        ...
        if rule.rule_value != p_external_tag:
          ...
          response.append("Repository mirror '%s' source tag updated to '%s'" % (name, p_external_tag))


        if config.internal_robot != robot:
          config_changed = True
          config.internal_robot = robot
          response.append("Repository mirror '%s' robot updated to '%s'" % (name, p_internal_robot))

        if config.external_registry != p_external_registry:
          config_changed = True
          config.external_registry = p_external_registry
          response.append("Repository mirror '%s' source registry updated to '%s'" % (name, p_external_registry))
        if config.external_namespace != p_external_namespace:
          config_changed = True
          config.external_namespace = p_external_namespace
          response.append("Repository mirror '%s' source namespace updated to '%s'" % (name, p_external_namespace))
        if config.external_repository != p_external_repository:
          config_changed = True
          config.external_repository = p_external_repository
          response.append("Repository mirror '%s' source repository updated to '%s'" % (name, p_external_repository))
        if config.external_registry_username != p_external_user:
          config_changed = True
          config.external_registry_username = p_external_user
          response.append("Repository mirror '%s' source username updated to '%s'" % (name, p_external_user))
        if config.external_registry_password != p_external_password:
          config_changed = True
          config.external_registry_password = p_external_password
          response.append("Repository mirror '%s' source password updated" % (name))
        if p_sync_start_date:
          if config.sync_start_date != p_sync_start_date:
            config_changed = True
            config.sync_start_date = p_sync_start_date
          response.append("Repository mirror '%s' sync start date updated" % (name))
        if config.is_enabled != p_is_enabled:
          config_changed = True
          config.is_enabled = p_is_enabled
          response.append("Repository mirror '%s' enabled flag updated" % (name))
